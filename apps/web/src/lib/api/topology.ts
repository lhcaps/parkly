...
export function getSites() {
  return apiFetch<SiteListRes>('/api/sites', undefined, (value) => {
...
export function getGates(siteCode: string) {
  ...
  return apiFetch<GateListRes>(`/api/gates${qs ? `?${qs}` : ''}`, undefined, (value) => {
    const row = isRecord(value) ? value : {}
    return {
...
export function getLanes(siteCode: string) {
  const qs = buildQuery({ siteCode })
  return apiFetch<LaneListRes>(`/api/lanes${qs ? `?${qs}` : ''}`, undefined, (value) => {
    const row = isRecord(value) ? value : {}
    return {
      siteCode: typeof row.siteCode === 'string' ? row.siteCode : siteCode,
...
export function getReportsSummary(siteCode: string, days = 7) {
  ...
  return apiFetch<ReportsSummaryRes>(`/api/reports/summary${qs ? `?${qs}` : ''}`, undefined, (value) => {
    const row = isRecord(value) ? value : {}
    return {
      siteCode: typeof row.siteCode === 'string' ? row.siteCode : siteCode,
