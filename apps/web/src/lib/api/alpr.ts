...
export function uploadImage(file: File, options?: AlprRequestOptions & { siteCode?: string }) {
  const fd = new FormData()
  fd.append('file', file)
  if (options?.siteCode) fd.append('siteCode', options.siteCode)
  return apiFetch<UploadedImageRes>('/api/media/upload', {
    ...
    const row = isRecord(value) ? value : {}
    return {
      imageUrl: typeof row.viewUrl === 'string' && row.viewUrl
        ? row.viewUrl
      ...
      size: typeof row.sizeBytes === 'number' && Number.isFinite(row.sizeBytes) ? row.sizeBytes : 0,
