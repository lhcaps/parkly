...
function withIdempotencyKey(headers: Headers, key?: string | null): Headers {
  const normalized = typeof key === 'string' && key.trim().length >= 8 ? key.trim() : null
  if (normalized) {
    headers.set('x-idempotency-key', normalized)
  }
  return headers
}
    ...
    refreshPromise = (async () => {
      ...
      .catch((error) => {
        ...
        if (normalized.status === 401) {
          invalidateAuthSession('refresh-401', {
...
export function postJson<T>(path: string, body: unknown, normalize?: (value: unknown) => T, idempotencyKey?: string | null): Promise<T> {
  const headers = withIdempotencyKey(jsonHeaders(), idempotencyKey ?? generateIdempotencyKey())
...
export function putJson<T>(path: string, body: unknown, normalize?: (value: unknown) => T, idempotencyKey?: string | null): Promise<T> {
  const headers = withIdempotencyKey(jsonHeaders(), idempotencyKey ?? generateIdempotencyKey())
  return apiFetch<T>(path, {
    method: 'PUT',
    headers,
...
export function patchJson<T>(path: string, body: unknown, normalize?: (value: unknown) => T, idempotencyKey?: string | null): Promise<T> {
  const headers = withIdempotencyKey(jsonHeaders(), idempotencyKey ?? generateIdempotencyKey())
  return apiFetch<T>(path, {
    method: 'PATCH',
    headers,
