...
export function AuditViewerPage() {
  ...
  async function load(overrideCursorKey?: string) {
    ...
    try {
      ...
      setCursorMap((prev) => ({
        ...prev,
        [targetCursorKey]: cursor,
        ...(auditRes.nextCursor ? { [`${targetCursorKey}:next`]: auditRes.nextCursor } : {}),
      }))
      setError('')
    } catch (loadError) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t('auditViewerPage.title')}
        description={t('auditViewerPage.description')}
        badges={[
          { label: t('auditViewerPage.badges.rows', { count: filteredRows.length }), variant: 'muted' },
          { label: t('auditViewerPage.badges.requestLinked', { count: requestLinked }), variant: requestLinked > 0 ? 'secondary' : 'outline' },
          { label: t('auditViewerPage.badges.correlationLinked', { count: correlationLinked }), variant: correlationLinked > 0 ? 'secondary' : 'outline' },
          { label: t('auditViewerPage.badges.manual', { count: manualLinked }), variant: manualLinked > 0 ? 'amber' : 'outline' },
        ]}
        actions={
        ...
        <Card className="border-border/80 bg-card/95">
          <CardContent className="pt-5">
            <div className="mb-2 flex items-center gap-2">
              ...
              <p className="font-medium">{t('auditViewerPage.cards.correlationLookup.title')}</p>
            </div>
            <p className="text-sm text-muted-foreground">{t('auditViewerPage.cards.correlationLookup.description')}</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/80 bg-card/95">
          <CardContent className="pt-5">
            <div className="mb-2 flex items-center gap-2">
              ...
              <p className="font-medium">{t('auditViewerPage.cards.beforeAfter.title')}</p>
            </div>
            <p className="text-sm text-muted-foreground">{t('auditViewerPage.cards.beforeAfter.description')}</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-border/80 bg-card/95">
          <CardContent className="pt-5">
            <div className="mb-2 flex items-center gap-2">
              ...
              <p className="font-medium">{t('auditViewerPage.cards.handoff.title')}</p>
            </div>
            <p className="text-sm text-muted-foreground">{t('auditViewerPage.cards.handoff.description')}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
        <Card className="border-border/80 bg-card/95">
          <CardHeader>
            <CardTitle>{t('auditViewerPage.list.title')}</CardTitle>
            <CardDescription>{t('auditViewerPage.list.description')}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex flex-wrap gap-2 text-[11px] font-mono-data text-muted-foreground">
              <Badge variant="outline">{t('auditViewerPage.list.window', { count: rows.length })}</Badge>
              <Badge variant="outline">{t('auditViewerPage.list.visible', { count: filteredRows.length })}</Badge>
              <Badge variant={manualLinked > 0 ? 'amber' : 'outline'}>{t('auditViewerPage.badges.manual', { count: manualLinked })}</Badge>
              <Badge variant={correlationLinked > 0 ? 'secondary' : 'outline'}>{t('auditViewerPage.badges.correlationLinked', { count: correlationLinked })}</Badge>
            </div>

            {loading ? (
              <SurfaceState title={t('auditViewerPage.list.loading')} tone="loading" className="min-h-[220px]" />
            ) : filteredRows.length === 0 ? (
              <SurfaceState title={t('auditViewerPage.list.emptyTitle')} description={t('auditViewerPage.list.emptyDescription')} className="min-h-[220px]" />
            ) : (
              <AuditTable rows={filteredRows} selectedAuditId={selectedAuditId} onSelect={handleSelect} />
            )}

            <div className="flex items-center justify-between gap-3 border-t border-border/70 pt-4">
              <Button variant="outline" size="sm" onClick={() => void handlePage('prev')} disabled={loading || cursorKey === ROOT_CURSOR}>
                {t('auditViewerPage.pagination.previous')}
              </Button>
              <p className="text-xs text-muted-foreground">
                {t('auditViewerPage.pagination.window', { value: cursorKey === ROOT_CURSOR ? t('auditViewerPage.pagination.root') : cursorKey })}
              </p>
              <Button variant="outline" size="sm" onClick={() => void handlePage('next')} disabled={loading || !nextCursor}>
                {t('auditViewerPage.pagination.next')}
              </Button>
            </div>
          </CardContent>
        </Card>
