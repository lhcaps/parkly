    ...
    <div className="animate-fade-in space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">This page reads master data from /sites + /gates + /lanes và inventory thật từ /api/devices để đối chiếu mapping của từng lane.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1.1fr_360px]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Source of truth: gate_lanes + gate_lane_devices. /api/devices returns inventoryà heartbeat mới nhất.</CardDescription>
          </CardHeader>
          <Separator />
          <CardContent className="space-y-4 pt-5">
            {loading ? (
              ...
              <div className="rounded-lg border border-dashed border-border px-4 py-10 text-center text-sm text-muted-foreground">None topology để hiển thị.</div>
            ) : (
              topology.map((item) => (
        ...
        <div className="space-y-5">
          <Card>
            <CardHeader>
              ...
              <CardDescription>This section is in sync with the current monorepo.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 text-sm text-foreground/85">
              <div className="rounded-lg border border-border bg-muted/20 px-4 py-3">
                ...
                <p className="mt-1 text-xs text-muted-foreground">API /health last response: {health || '—'}</p>
              </div>
              <div className="rounded-lg border border-border bg-muted/20 px-4 py-3">
                <div className="flex items-center gap-2">
                  <Cpu className="h-4 w-4 text-muted-foreground" />
                  <p className="font-medium">GET /api/devices is live</p>
                </div>
                <p className="mt-1 text-xs text-muted-foreground">This page calls /api/devices per site to lấy inventory và heartbeat mới nhất.</p>
              </div>
              <div className="rounded-lg border border-border bg-muted/20 px-4 py-3">
                <div className="flex items-center gap-2">
                  <Activity className="h-4 w-4 text-muted-foreground" />
                  <p className="font-medium">Heartbeat capture route is mounted</p>
                </div>
                <p className="mt-1 text-xs text-muted-foreground">Devices that have sent a heartbeat will show status, age và latency ngay trên lane card.</p>
              </div>
              <div className="rounded-lg border border-border bg-muted/20 px-4 py-3">
                <div className="flex items-center gap-2">
                  <MapPinned className="h-4 w-4 text-muted-foreground" />
                  <p className="font-medium">Lane is the primary aggregate</p>
                </div>
                <p className="mt-1 text-xs text-muted-foreground">gate_lanes determines routing, gate_lane_devices maps camera/RFID/loop/barrier vào lane tương ứng.</p>
              </div>
              <div className="rounded-lg border border-border bg-muted/20 px-4 py-3">
                <div className="flex items-center gap-2">
                  <Unplug className="h-4 w-4 text-muted-foreground" />
                  <p className="font-medium">Console links</p>
                </div>
                <p className="mt-1 text-xs text-muted-foreground">Lane aggregate health, SSE realtime, and outbox areó ở các màn hình chuyên biệt: Lane Monitor, Device Health và Outbox Monitor.</p>
              </div>
            </CardContent>
          </Card>
