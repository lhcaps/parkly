...
export function OutboxMonitorPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Sync Outbox"
        description="Investigate the sync queue with stable filters, clear open records, and direct audit handoff."
        badges={[
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Link to="/review-queue">
          <Card className="ops-detail-card h-full transition hover:border-primary/30 hover:bg-primary/5">
            <CardContent className="pt-5">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Check inputs when the backlog originates from cases needing special attention.át từ các trường hợp cần xác nhận thủ công.</p>
            </CardContent>
          </Card>
        </Link>
        ...
        <Link to="/lane-monitor">
          <Card className="ops-detail-card h-full transition hover:border-primary/30 hover:bg-primary/5">
            <CardContent className="pt-5">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Cross-reference which lanes generated events that haven't synced downstream.kiện nhưng chưa đồng bộ ra ngoài.</p>
            </CardContent>
          </Card>
        </Link>
        ...
        <Card className="ops-detail-card h-full">
          <CardContent className="pt-5">
            ...
            <p className="text-sm text-muted-foreground">This screen is for sync queue operations. Lane processing remains in Run Lane.g bộ. Xử lý lane vẫn nên thực hiện ở các màn vận hành tương ứng.</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        ...
        <Card className="ops-detail-card">
          <CardHeader>
            ...
            <CardDescription>Requeue — OPS or ADMIN. Drain — ADMIN or WORKER.</CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-wrap gap-2">
              ...
              <DangerConfirmDialog
                ...
                description="Returns up to 50 recent records to pending for re-inspection. để bạn kiểm tra lại đường đồng bộ."
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,0.56fr)_minmax(360px,0.44fr)]">
        <Card className="ops-detail-card">
          <CardHeader>
            ...
            <CardDescription>Authoritative REST list. Share the URL, select records to investigate, and reconcile with the stream snapshot.chiếu với stream snapshot ở panel bên cạnh.</CardDescription>
          </CardHeader>
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                          {row.siteCode || '—'} / {row.laneCode || '—'} / {row.deviceCode || '—'} · {new Date(row.updatedAt).toLocaleString('vi-VN')}
                        </p>
                      </div>
