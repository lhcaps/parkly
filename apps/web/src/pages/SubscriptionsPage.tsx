  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title={t('route.subscriptions.label')}
        description={t('route.subscriptions.description')}
        badges={[
          { label: t('subscriptionsPage.badges.canonical'), variant: 'secondary' },
          { label: workspace.operatorRole || t('common.dash'), variant: 'muted' },
          {
            label: workspace.canMutate ? t('subscriptionsPage.badges.mutable') : t('subscriptionsPage.badges.readOnly'),
            variant: workspace.canMutate ? 'entry' : 'outline',
          },
        ]}
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              variant="outline"
              size="lg"
              onClick={() => void workspace.reloadList()}
              disabled={workspace.list.loading || mutations.state.busy}
      ...
      {mutations.state.error && mutations.state.action.toLowerCase().includes('create subscription') ? (
        <StateBanner error={mutations.state.error} title={t('subscriptionsPage.createFailed')} />
      ) : null}
