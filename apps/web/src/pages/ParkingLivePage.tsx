  ...
  return (
    <div className="space-y-5">
      <PageHeader
        eyebrow="Monitoring"
        title="Parking Live"
        description="Realtime slot occupancy by floor. The board is tuned for scanability first: where the attention is, which zone is impacted, and which slot should be opened next."
        badges={[
          { label: freshness.status === 'connected' ? 'realtime + poll' : freshness.status === 'stale' ? 'snapshot fallback' : 'resilience mode', variant: 'outline' },
