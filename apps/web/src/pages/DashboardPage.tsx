  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-4xl text-sm text-muted-foreground">
            Operations console: lane monitor, review queue, session history, device health, and outbox monitor. và outbox monitor; toàn bộ panel bám theo API và SSE thật.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Lane Monitor" helper="Lane aggregate health, barrier status, and latest session." icon={Activity} />
        <MetricCard label="Review Queue" helper="Claim reviews, manual approve/reject/open barrier with audit trail." icon={ClipboardCheck} />
        <MetricCard label="Device Health" helper="Heartbeat age, online/degraded/offline by device." icon={Cpu} />
        <MetricCard label="Outbox Monitor" helper="Monitor the outbox feed and retry activity." icon={RadioTower} />
      </div>
      ...
      <Card className="border-border/80 bg-card/95">
        <CardContent className="flex items-start gap-3 pt-5">
          ...
          <div>
            <p className="font-medium">Session detail — in Session History</p>
            <p className="mt-1 text-sm text-muted-foreground">
              From there, read timeline, decisions, barrier commands, evidence, and manual actions. action phù hợp với allowedActions cùng role hiện tại.
            </p>
          </div>
        </CardContent>
      </Card>
