...
export function AppTopbar({ onOpenSidebar }: AppTopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-10 border-b border-border/75 bg-background/78 shadow-[0_10px_30px_rgba(34,92,136,0.06)] backdrop-blur-2xl">
      <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4 px-4 py-0 sm:px-6 lg:px-8" style={{ height: '56px' }}>
          ...
          <div className="min-w-0">
            ...
            {principal ? (
              <p className="mt-0.5 truncate text-[12px] text-muted-foreground">
                {roleLabels.focusLabel} / {siteScopeLabel}
              </p>
            ) : null}
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-3">
          {displayName ? (
            <div className="flex items-center gap-2 rounded-2xl border border-border/60 bg-card/82 px-3 py-1.5 shadow-[0_12px_28px_rgba(35,94,138,0.09)]">
              <span
          ...
          {principal ? (
            <Button
              ...
              variant="ghost"
              size="sm"
              className="h-8 gap-1.5 rounded-xl border border-transparent bg-background/40 text-muted-foreground hover:border-border/70 hover:bg-card/80 hover:text-foreground"
