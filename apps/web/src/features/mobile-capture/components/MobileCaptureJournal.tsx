...
function formatDateTime(value: string) {
  ...
  return date.toLocaleString('vi-VN', { timeStyle: 'short', dateStyle: 'short' })
}
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline justify-between gap-2">
            ...
            <span className="shrink-0 text-[10px] font-mono-data text-muted-foreground">{formatDateTime(row.ts)}</span>
          </div>
          {detailPreview ? (
    ...
    <Card className="border-border/80 bg-card/95">
      <CardHeader className="pb-2">
        <div className="flex items-start justify-between gap-3">
          <div>
            <CardTitle className="text-base">Ops journal</CardTitle>
            <CardDescription className="text-xs">Log preview, heartbeat và capture gần nhất.</CardDescription>
          </div>
