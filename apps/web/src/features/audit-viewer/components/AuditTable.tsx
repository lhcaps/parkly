      ...
      <table className="min-w-full divide-y divide-border text-sm">
        <thead>
          <tr className="text-left text-[11px] font-mono-data uppercase tracking-[0.18em] text-muted-foreground">
            <th className="px-4 py-3">{t('auditViewer.table.audit')}</th>
            <th className="px-4 py-3">{t('auditViewer.table.action')}</th>
            <th className="px-4 py-3">{t('auditViewer.table.actor')}</th>
            <th className="px-4 py-3">{t('auditViewer.table.entity')}</th>
            <th className="px-4 py-3">{t('auditViewer.table.correlation')}</th>
            <th className="px-4 py-3">{t('auditViewer.table.occurred')}</th>
            <th className="px-4 py-3 text-right">{t('auditViewer.table.open')}</th>
          </tr>
        </thead>
            ...
            return (
              <tr key={row.auditId} className={cn('perf-list-item transition-colors', selected ? 'bg-primary/6' : 'hover:bg-muted/20')}>
                <td className="px-4 py-3 align-top">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">{row.siteCode || t('common.dash')}</p>
                </td>
                <td className="px-4 py-3 align-top">
                  ...
                  {summary.changedKeys.length > 0 ? (
                    <p className="mt-2 max-w-[280px] text-xs text-muted-foreground">
                      {t('auditViewer.table.changedPrefix', { value: summary.changedKeys.join(', ') })}
                    </p>
                  ) : (
                    <p className="mt-2 text-xs text-muted-foreground">{t('auditViewer.table.noDiffKeys')}</p>
                  )}
                </td>
                <td className="px-4 py-3 align-top">
                  <p className="text-sm font-medium text-foreground">{actorHeadline(row.actor, row.actorUserId)}</p>
                  <p className="mt-1 text-xs text-muted-foreground">{row.actor?.role || row.actor?.principalType || t('common.dash')}</p>
                </td>
                <td className="px-4 py-3 align-top">
                  <p className="text-sm text-foreground">{entityHeadline(row)}</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t('auditViewer.table.beforeAfter', { before: summary.beforeKeys, after: summary.afterKeys })}
                  </p>
                </td>
                <td className="px-4 py-3 align-top">
                  <p className="max-w-[180px] break-all text-xs text-foreground">{row.correlationId || row.requestId || t('common.dash')}</p>
                </td>
                ...
                <td className="px-4 py-3 align-top text-right">
                  <Button variant={selected ? 'secondary' : 'outline'} size="sm" onClick={() => onSelect(row)}>
                    {selected ? t('auditViewer.table.opened') : t('auditViewer.table.open')}
                  </Button>
                </td>
              </tr>
            )
