...
export function ActivePairsTable({
  ...
  return (
    <div className="rounded-3xl border border-border/80 bg-card/95 p-5 shadow-[0_18px_60px_rgba(35,94,138,0.12)]">
      <div className="mb-4">
        <p className="text-sm font-medium">{t('mobilePairPage.activePairs.title')}</p>
        <p className="mt-1 text-xs text-muted-foreground">{t('mobilePairPage.activePairs.description')}</p>
      </div>
      ...
      {rows.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border/80 bg-background/40 px-4 py-10 text-center text-sm text-muted-foreground">
          {t('mobilePairPage.activePairs.empty')}
        </div>
      ) : (
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-2 break-all font-mono-data text-[11px] text-muted-foreground">
                      {t('mobilePairPage.activePairs.origin', { value: row.pairOrigin || t('common.dash') })} · {t('mobilePairPage.activePairs.registry', { version: row.registryVersion })}
                      {row.migratedAt ? ` · ${t('mobilePairPage.activePairs.migrated', { value: new Date(row.migratedAt).toLocaleString('vi-VN') })}` : ''}
                    </p>
                    ...
                    <p className="mt-2 text-[11px] font-mono-data text-muted-foreground">
                      {t('mobilePairPage.activePairs.createdAndOpened', {
                        created: new Date(row.createdAt).toLocaleString('vi-VN'),
                        opened: new Date(row.lastOpenedAt).toLocaleString('vi-VN'),
                      })}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button type="button" variant="outline" size="sm" onClick={() => onCopy(row)}>
                      {t('mobilePairPage.activePairs.copy')}
                    </Button>
                    <Button type="button" size="sm" onClick={() => onOpen(row)}>
                      {t('mobilePairPage.activePairs.open')}
                    </Button>
                    <ConfirmActionButton
                      variant="ghost"
                      size="sm"
                      confirmTitle={t('mobilePairPage.activePairs.removeTitle')}
                      confirmDescription={t('mobilePairPage.activePairs.removeDescription')}
                      onConfirm={() => onRemove(row.pairId)}
                    >
                      <Trash2 className="h-4 w-4" />
                      {t('mobilePairPage.activePairs.remove')}
                    </ConfirmActionButton>
                  </div>
                </div>
              </div>
