...
export function SessionDetailConsole({ detail }: { detail: SessionDetail }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
      <Section title="Reads" description="Read events by time and source device." empty="None read event.">
        {detail.reads.length === 0 ? <p className="text-sm text-muted-foreground">None read event.</p> : (
      ...
      <Section title="Decisions" description="Decision log with reason code and detail clearly separated." empty="— decision.">
        {detail.decisions.length === 0 ? <p className="text-sm text-muted-foreground">— decision.</p> : (
          <div className="space-y-3">
      ...
      <Section title="Barrier commands" description="Barrier lifecycle and command result." empty="— barrier command.">
        {detail.barrierCommands.length === 0 ? <p className="text-sm text-muted-foreground">— barrier command.</p> : (
          <div className="space-y-3">
      ...
      <Section title="Manual reviews" description="Review queue lineage linked to this session." empty="— manual review.">
        {detail.manualReviews.length === 0 ? <p className="text-sm text-muted-foreground">— manual review.</p> : (
          <div className="space-y-3">
      ...
      <Section title="Evidence / media" description="Media references collected into a readable surface." empty="— media evidence.">
        {mediaRows.length === 0 ? <p className="text-sm text-muted-foreground">— media evidence.</p> : (
          <div className="space-y-3">
            {mediaRows.map((read) => (
      ...
      <Section title="Incidents" description="Incident surface separated from the raw snapshot." empty="— incident.">
        {detail.incidents.length === 0 ? <p className="text-sm text-muted-foreground">— incident.</p> : (
          <div className="space-y-3">
