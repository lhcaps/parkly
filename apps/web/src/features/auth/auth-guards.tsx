...
import { canAccessRoute, getForbiddenFallbackPath, getRoutePolicy, getRoleHome } from '@/lib/auth/role-policy'
...
export function RequireRouteRole({ routePath, children }: { routePath: string; children: ReactNode }) {
  ...
  if (!canAccessRoute(auth.principal.role, routePolicy)) {
    return <Navigate to="/forbidden" replace state={{ from: location, requestedPath, fallbackPath, requiredRoles: routePolicy.allowedRoles ?? [] }} />
  }

  const requiresSiteScope = routePolicy.requiresSiteScope
  const missingSiteScope = requiresSiteScope
    && auth.principal.principalType === 'USER'
    && auth.principal.role !== 'SUPER_ADMIN'
    && auth.principal.siteScopes.length === 0
