...
export function DeveloperDebugTab({
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1.3fr_0.9fr]">
      <Card className="border-border/80 bg-card/95 shadow-[0_16px_48px_rgba(0,0,0,0.14)]">
        ...
        <CardContent className="space-y-4 text-sm">
          <div className="grid gap-3 sm:grid-cols-2">
            <Metric label="API base" value={apiBase || 'Không có'} />
            <Metric label="Token" value={tokenPreview || 'Chưa cấu hình'} />
          ...
          <div className="space-y-3">
            <p className="font-medium text-foreground">Default context hiện tại</p>
      ...
      <Card className="border-border/80 bg-card/95 shadow-[0_16px_48px_rgba(0,0,0,0.14)]">
        <CardHeader>
          <CardTitle className="text-base font-semibold tracking-tight">Dọn cache cục bộ</CardTitle>
        </CardHeader>
          ...
          <div className="rounded-2xl border border-border/70 bg-muted/35 p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {cacheKeys.length ? (
                ...
                <p className="text-muted-foreground">Không có key nào của app trong localStorage.</p>
              )}
            </div>
          </div>
        </CardContent>
