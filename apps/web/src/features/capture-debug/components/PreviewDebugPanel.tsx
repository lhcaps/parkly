...
import { AlertCircle, CheckCircle2, Loader2, ScanSearch, ShieldAlert, Upload } from 'lucide-react'
...
export function PreviewDebugPanel() {
  ...
  async function handleRun() {
    if (!file) {
      setError('Chọn ảnh trước khi chạy preview debug.')
      return
    }
  ...
  return (
    <Card className="border-border/80 bg-card/95 shadow-[0_18px_60px_rgba(0,0,0,0.12)]">
      <CardHeader>
        ...
        <CardDescription>
          Dùng surface này để nhìn raw OCR, candidates, crop/psm/winner và provider diagnostics mà không đụng Run Lane.
        </CardDescription>
      </CardHeader>
          ...
          <div className="space-y-4">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div className="rounded-2xl border border-border/80 bg-background/40 p-4">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {result.needsConfirm ? <Badge variant="amber"><ShieldAlert className="h-3 w-3" />needs confirm</Badge> : <Badge variant="secondary"><CheckCircle2 className="h-3 w-3" />ready</Badge>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-border/80 bg-background/40 p-4">
              ...
              {result.candidates.length === 0 ? (
                <p className="text-sm text-muted-foreground">Không có candidate nào.</p>
              ) : (
            ...
            <div className="rounded-3xl border border-border/80 bg-background/40 p-4">
              ...
              {diagnostics.length === 0 ? (
                <p className="mt-2 text-sm text-muted-foreground">Không có diagnostics tóm tắt.</p>
              ) : (
