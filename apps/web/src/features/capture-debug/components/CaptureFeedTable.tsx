...
export function CaptureFeedTable({
  ...
  const { items, state, resync } = useSseFeed<GateEventStreamItem>({
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            This feed tracks real ingest events. It does not affect Run Lane state.
          </p>
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <Badge variant={state.connected ? 'entry' : state.stale ? 'amber' : 'outline'}>
            {state.connected ? <Wifi className="h-3 w-3" /> : <WifiOff className="h-3 w-3" />}
            {state.status}
          </Badge>
          <Button type="button" size="sm" variant="outline" onClick={resync}>
            {state.status === 'reconnecting' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}
            Resync
          </Button>
        </div>
      </div>

      <RealtimeStatusBanner title="Gate event stream" state={state} onResync={resync} />

      {items.length === 0 ? (
        <div className="mt-4 rounded-2xl border border-dashed border-border/80 bg-background/40 px-4 py-12 text-center text-sm text-muted-foreground">
          No events from the gate event stream.
        </div>
      ) : (
        <ScrollArea className="mt-4 h-[560px] pr-3">
