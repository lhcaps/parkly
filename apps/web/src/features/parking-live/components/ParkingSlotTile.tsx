...
const STATUS_CLASSES: Record<string, string> = {
  EMPTY:
    'border-success/25 bg-success/8 text-success hover:border-success/50 hover:bg-success/14',
  OCCUPIED_MATCHED:
    'border-primary/30 bg-primary/10 text-primary hover:border-primary/50 hover:bg-primary/16',
  OCCUPIED_UNKNOWN:
    'border-amber-500/30 bg-amber-500/10 text-amber-400 hover:border-amber-500/50',
  OCCUPIED_VIOLATION:
    'border-destructive/35 bg-destructive/12 text-destructive hover:border-destructive/55',
  SENSOR_STALE:
  ...
  return (
    <button
      ...
      className={cn(
        ...
        slot.isStale && 'opacity-60',
      )}
    >
      {/* Top: spot code + status dot */}
      <div className="flex items-center justify-between gap-1">
        <span className="font-mono-data text-[11px] font-medium leading-none truncate">
          {slot.spotCode}
        </span>
        <span
          className={cn(
      ...
      {/* Bottom: plate or status hint */}
      <div className="mt-1">
        {slot.observedPlate ? (
          <span className="font-mono-data text-[10px] leading-none opacity-80">
            {slot.observedPlate}
          </span>
      ...
      {slot.hasSubscription ? (
        <span className="absolute right-1.5 top-1 text-[8px] font-mono-data uppercase tracking-wide opacity-60">
          sub
        </span>
      ) : null}
    </button>
