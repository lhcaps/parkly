...
export const ParkingLiveSummaryBar = memo(function ParkingLiveSummaryBar({ summary }: Props) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-0 divide-x divide-border/60 rounded-2xl border border-border/60 bg-card/70 px-1">
      ...
      <Metric label="Occupied" value={summary.occupied} accent={occupancyPct >= 85 ? 'text-destructive' : occupancyPct >= 60 ? 'text-amber-400' : 'text-primary'} />
      {summary.occupiedViolation > 0 ? (
        <Metric label="Violation" value={summary.occupiedViolation} accent="text-destructive" />
      ) : null}
      {summary.occupiedUnknown > 0 ? (
        <Metric label="Unknown" value={summary.occupiedUnknown} accent="text-amber-400" />
      ) : null}
      {summary.stale > 0 ? (
        <Metric label="Stale" value={summary.stale} accent="text-muted-foreground" />
      ) : null}
      <div className="flex flex-col items-center gap-0.5 px-3 py-2">
        <span
          className={`font-mono-data text-lg font-semibold tabular-nums ${
