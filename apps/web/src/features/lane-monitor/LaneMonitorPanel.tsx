    ...
    <Card className="border-border/80 bg-card/95">
      <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
        <div>
          ...
          <CardDescription>Monitor lane health, barrier status, and devices directly affecting vehicle flow.ận hành.</CardDescription>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        {state.error && rows.length === 0 ? (
          ...
          <SurfaceState
            ...
            description="The backend pushes the full lane snapshot via SSE when available. Operators can manually resync to reopen the stream.anual resync để ép hydrate lại snapshot REST."
            icon={Layers}
