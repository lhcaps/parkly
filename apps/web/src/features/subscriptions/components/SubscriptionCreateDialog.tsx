...
export function SubscriptionCreateDialog({
  ...
  const siteOptions = useMemo<SelectOption[]>(() => {
    return [
      { value: '', label: 'Select site', description: 'All sites' },
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!form.siteCode.trim()) {
      setValidationError('Please select a site.')
      return
    }
    if (!form.customerId.trim()) {
      setValidationError("Enter an existing customer's customer_id, phone number, or email.")
      return
    }
  ...
  return (
    <>
      ...
      <SubscriptionEditorDialogShell
        ...
        description="Create a new parking plan for the site, customer, plan type, and effective date window."
        submitLabel="Create plan"
        busy={busy}
        onClose={() => setOpen(false)}
        onSubmit={handleSubmit}
        footer="This form resolves an existing customer by customer_id, phone number, or email. It does not create a new customer inline."
      >
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <div className="space-y-2.5">
              ...
              <Input
                ...
                placeholder="Enter customer_id, phone number, or email"
                disabled={busy}
                className="h-12 text-base"
              />
              <p className="text-xs text-muted-foreground">
                Use the customer_id, phone number, or email of an existing customer.
              </p>
            </div>
