  ...
  return (
    <Card className="border-border/80 bg-card/95">
      <CardHeader>
        ...
        <CardDescription>
          Final action, reason code, reason detail, and review requirement are immediately visible.
        </CardDescription>
      </CardHeader>
      ...
      <CardContent>
        {decision ? (
          ...
          <div className="flex min-h-[180px] flex-col items-center justify-center rounded-3xl border border-dashed border-border/80 bg-background/40 px-6 py-8 text-center">
            ...
            <p className="text-sm font-medium">— decision result</p>
            <p className="mt-1 text-xs text-muted-foreground">
              Decision summary will appear after a successful lane flow submission.
            </p>
          </div>
        )}
        ...
        {decision?.reviewRequired ? (
          <div className="mt-4 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-muted-foreground">
            <div className="mb-2 flex items-center gap-2 text-foreground">
              ...
              <span className="font-medium">Review flag active</span>
            </div>
            The UI pins reviewRequired on the result surface so operators cannot miss cases that need the review queue.
          </div>
        ) : decision ? (
          <div className="mt-4 rounded-2xl border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground">
            ...
            Current decision does not require further review.
          </div>
        ) : null}
      </CardContent>
    </Card>
