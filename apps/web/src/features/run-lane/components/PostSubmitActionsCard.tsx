  ...
  return (
    <Card className="border-border/80 bg-card/95">
      <CardHeader>
        ...
        <CardDescription>
          Operator phải tiếp tục xử lý confirm-pass, cancel-session hoặc refresh session detail ngay tại đây, không cần rời màn.
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="rounded-3xl border border-border/80 bg-background/40 p-4">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Effective plate hiện tại: <span className="font-mono-data text-foreground">{effectivePlate || '—'}</span>
          </p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-2xl border border-border/80 bg-muted/25 p-4 text-sm text-muted-foreground">
            ...
            {allowedActions.length > 0 ? allowedActions.join(', ') : 'Chưa có allowed actions hoặc session chưa được hydrate.'}
          </div>
          ...
          <div className="rounded-2xl border border-border/80 bg-muted/25 p-4">
            <Button
              ...
              onClick={() => resetSubmitResult('Result surface đã được reset thủ công. Preview và override hiện tại vẫn được giữ nguyên.')}
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              Reset result không đụng preview và override. Đây là chỗ để operator chuẩn bị submit lượt mới mà không mất dữ liệu capture hiện tại.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-sm text-muted-foreground">
          ...
          Submit lỗi thì preview và override không bị reset. Submit thành công thì decision/session/event được chốt ngay trên surface này để operator đọc và hành động tiếp.
        </div>
      </CardContent>
    </Card>
  )
