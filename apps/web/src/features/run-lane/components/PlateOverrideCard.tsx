  ...
  return (
    <Card className="border-border/80 bg-card/95">
      <CardHeader>
        ...
        <CardDescription>
          Preview và manual override chạy song song. Input này không bị disable khi backend preview đang loading.
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Preview đầu tiên sẽ auto-fill khi operator chưa chạm tay. Sau khi đã gõ tay, preview mới không được phép đè giá trị này.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {override.sourceMode === 'manual_override' ? <Badge variant="amber">override thủ công đang bật</Badge> : null}
          {override.sourceMode === 'backend_preview' ? <Badge variant="muted">đang dùng backend preview</Badge> : null}
          {preview.stage === 'loading' || preview.stage === 'uploading' ? <Badge variant="outline">preview đang chạy nhưng input vẫn mở</Badge> : null}
        </div>
        ...
        <div className="rounded-2xl border border-border/80 bg-muted/25 p-4 text-sm text-muted-foreground">
          ...
          Auto-fill chỉ xảy ra khi <span className="font-mono-data">override.touched = false</span>. Sau khi operator đã động tay vào input,
          preview mới chỉ cập nhật candidate/backend result, không được đè input nữa.
        </div>
      </CardContent>
    </Card>
  )
