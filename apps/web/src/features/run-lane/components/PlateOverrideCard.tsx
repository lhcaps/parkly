      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            First preview auto-fills when untouched. After manual input, new previews cannot overwrite it.phép đè giá trị này.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {preview.stage === 'loading' || preview.stage === 'uploading' ? <Badge variant="outline">preview loadingg chạy nhưng input vẫn mở</Badge> : null}
        </div>
        ...
        <div className="rounded-2xl border border-border/80 bg-muted/25 p-4 text-sm text-muted-foreground">
          ...
          Auto-fill only occurs when <span className="font-mono-data">override.touched = false</span>. Sau khi operator đã động tay vào input,
          new preview only updates candidates/backend result — it cannot overwrite the input.
        </div>
      </CardContent>
    </Card>
  )
