      ...
      <Card className="border-border/80 bg-card/95 shadow-[0_18px_60px_rgba(0,0,0,0.18)]">
        <CardHeader className="space-y-3">
          ...
          <div>
            ...
            <CardDescription>
              PR-10 biến panel phải thành result surface thực thụ: decision, session, event và post-submit actions đều nằm tại chỗ.
            </CardDescription>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="rounded-3xl border border-border/80 bg-muted/25 p-4">
            <SummaryRow label="Site" value={topology.siteCode || 'Chưa chọn'} />
            <SummaryRow label="Lane" value={topology.laneCode || 'Chưa chọn'} />
            <SummaryRow label="Effective plate" value={effectivePlate || 'Chưa có effective plate'} />
            <SummaryRow label="Plate source" value={effectiveSource} />
            <SummaryRow label="Current session" value={session?.sessionId ? String(session.sessionId) : 'Chưa có'} />
            <SummaryRow label="Decision code" value={decision?.decisionCode || 'Chưa có'} />
            <SummaryRow label="Gate event" value={event?.eventId != null ? String(event.eventId) : 'Chưa có'} />
          </div>
        </CardContent>
      </Card>
