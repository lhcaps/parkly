...
export function createRunLaneStore(initialState?: Partial<RunLaneStoreState>): RunLaneStoreApi {
  ...
  const actions: RunLaneStoreActions = {
    ...
    hydrateSites(sites, preferredSiteCode?: string) {
      setState((current) => {
        const preferred = preferredSiteCode?.trim()
        const nextSiteCode =
          preferred && sites.some((s) => s.siteCode === preferred)
            ? preferred
            : current.topology.siteCode && sites.some((s) => s.siteCode === current.topology.siteCode)
    ...
    hydrateSiteTopology(payload, preserveSelection?: { gateCode?: string; laneCode?: string }) {
      setState((current) => {
        ...
        const gateValid =
          preserveSelection?.gateCode &&
          payload.gates.some((g) => g.gateCode === preserveSelection.gateCode)
        const nextGateCode = gateValid
          ? preserveSelection!.gateCode!
          : resolveNextGateCode({ ...nextTopology, gateCode: current.topology.gateCode })

        const laneBelongsToGate =
          preserveSelection?.laneCode &&
          payload.lanes.some(
            (l) =>
              l.laneCode === preserveSelection.laneCode && l.gateCode === nextGateCode,
          )
        const nextLaneCode =
          laneBelongsToGate
            ? preserveSelection!.laneCode!
            : resolveNextLaneCode(nextTopology, nextGateCode)
    ...
    setGateCode(gateCode) {
      setState((current) => {
        ...
        return {
          topology: {
            ...nextTopology,
            laneCode: resolveNextLaneCode(nextTopology, gateCode),
          },
