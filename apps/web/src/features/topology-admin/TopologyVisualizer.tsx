...
const NODE_HEIGHT_GATE = 80
const NODE_HEIGHT_LANE = 100
const NODE_HEIGHT_DEVICE = 70
...
function GateNode({ data }: { data: { label: string; laneCount: number } }) {
  return (
    <div className="px-6 py-4 rounded-xl shadow-2xl border border-border/40 bg-[#151518]/90 backdrop-blur-md min-w-[280px] text-center group transition-all hover:border-primary/50 relative overflow-hidden">
      ...
      <div className="font-semibold text-base mb-2 text-foreground tracking-wide">{data.label}</div>
      <Badge variant="secondary" className="bg-muted text-muted-foreground hover:bg-muted font-normal uppercase tracking-wider text-[10px] px-2 py-0.5">
...
function LaneNode({ data }: { data: { lane: TopologyLane; onEdit: (laneId: string) => void } }) {
  ...
  return (
    <div className="px-5 py-4 rounded-xl shadow-xl border border-border/40 bg-[#1a1b1e]/95 backdrop-blur-md min-w-[260px] transition-all hover:border-primary/50 relative group">
      <Handle type="target" position={Position.Top} className="w-2 h-2 bg-muted-foreground border-none" />

      <div className="flex justify-between items-start mb-3">
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <span className="font-medium text-sm text-foreground/90">{lane.label}</span>
        ...
        <button
          ...
          className="text-muted-foreground hover:text-foreground transition-colors p-1 rounded-md opacity-0 group-hover:opacity-100"
        >
...
function DeviceNode({ data }: { data: { device: TopologyDevice; laneId: string } }) {
  ...
  return (
    <div
      className={`px-4 py-3 rounded-xl shadow-lg border border-border/40 bg-[#1c1d22]/95 min-w-[220px] cursor-pointer hover:border-amber-500/50 transition-all group ${
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="font-mono text-[13px] font-bold text-foreground/90 truncate max-w-[150px]" title={d.deviceCode}>
          {d.deviceCode}
        </div>
      ...
      <div className="flex items-center gap-2">
        <span className="text-[11px] font-medium text-muted-foreground capitalize truncate max-w-[100px]" title={d.deviceType}>
    ...
    <>
      <div className="h-full w-full" onDragOver={onDragOver} onDrop={onDrop}>
        ...
        >
          ...
          <Controls className="!bg-[#151518] !border-border/40 !fill-muted-foreground [&>button]:!border-border/40 hover:[&>button]:!bg-muted" />
          <MiniMap
            nodeColor={(node) => {
