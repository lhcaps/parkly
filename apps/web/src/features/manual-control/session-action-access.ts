...
export type OperatorRole = 'SUPER_ADMIN' | 'SITE_ADMIN' | 'MANAGER' | 'OPERATOR' | 'GUARD' | string
...
const SESSION_ACTION_ROLES: Record<SessionAllowedAction, readonly string[]> = {
  APPROVE: ['GUARD', 'SUPER_ADMIN', 'SITE_ADMIN', 'MANAGER', 'OPERATOR'],
  REQUIRE_PAYMENT: ['GUARD', 'SUPER_ADMIN', 'SITE_ADMIN', 'MANAGER', 'OPERATOR'],
  DENY: ['GUARD', 'SUPER_ADMIN', 'SITE_ADMIN', 'MANAGER', 'OPERATOR'],
  CONFIRM_PASS: ['GUARD', 'SUPER_ADMIN', 'SITE_ADMIN', 'MANAGER', 'OPERATOR'],
  CANCEL: ['SUPER_ADMIN', 'SITE_ADMIN', 'MANAGER', 'OPERATOR'],
}

const REVIEW_ACTION_ROLES: Record<ReviewQueueAction, readonly string[]> = {
  CLAIM: ['GUARD', 'SUPER_ADMIN', 'SITE_ADMIN', 'MANAGER', 'OPERATOR'],
  MANUAL_APPROVE: ['GUARD', 'SUPER_ADMIN', 'SITE_ADMIN', 'MANAGER', 'OPERATOR'],
  MANUAL_REJECT: ['GUARD', 'SUPER_ADMIN', 'SITE_ADMIN', 'MANAGER', 'OPERATOR'],
  MANUAL_OPEN_BARRIER: ['SUPER_ADMIN', 'SITE_ADMIN', 'MANAGER', 'OPERATOR'],
}
...
const MANUAL_BARRIER_OVERRIDE_ROLES: readonly string[] = ['SUPER_ADMIN', 'SITE_ADMIN', 'MANAGER', 'OPERATOR']
...
export function getManualBarrierOverrideLockReason(
  ...
  if (!canRunManualBarrierOverride(role)) {
    return `Role ${role} does not have the manual-open-barrier permission. Operator/admin shell required.`
  }
  return ''
}
