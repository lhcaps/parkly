...
import { AlertCircle, AlertTriangle, CheckCircle2, Inbox, Loader2, RefreshCcw, Wifi, WifiOff } from 'lucide-react'
...
export function ConnectionBadge({ connected, label }: { connected: boolean; label?: string }) {
  return (
    <div
      className={cn(
        'inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[11px] font-mono-data',
        connected ? 'border-success/25 bg-success/8 text-success' : 'border-border bg-card text-muted-foreground',
      )}
    >
      {connected ? <Wifi className="h-3.5 w-3.5" /> : <WifiOff className="h-3.5 w-3.5" />}
      {label ?? 'Stream'} {connected ? 'connected' : 'disconnected'}
    </div>
  )
}
