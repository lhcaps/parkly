    ...
    <div className={cn('rounded-2xl border border-border/70 bg-card/80', className)}>
      <div className="flex w-full items-center justify-between gap-3 px-4 py-3">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="flex flex-1 items-center gap-2 min-w-0 text-left transition-colors hover:bg-muted/25 rounded-md -mx-2 px-2 py-1"
        >
          ...
          {open
            ? <ChevronDown className="h-3.5 w-3.5 text-muted-foreground transition-transform duration-200 ml-auto shrink-0" />
            : <ChevronRight className="h-3.5 w-3.5 text-muted-foreground transition-transform duration-200 ml-auto shrink-0" />}
        </button>
        {headerAction ? <div className="shrink-0" onClick={(e) => e.stopPropagation()}>{headerAction}</div> : null}
      </div>
      {open && (
