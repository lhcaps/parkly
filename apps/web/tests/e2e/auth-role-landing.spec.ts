...
function userPrincipal(role: 'ADMIN' | 'OPS' | 'GUARD' | 'CASHIER' | 'WORKER') {
  return {
    principalType: 'USER',
    role,
...
test('forbidden direct URL keeps the user out of subscriptions and shows a guarded fallback', async ({ page }) => {
  ...
  await expect(page.getByText('Route blocked')).toBeVisible()
  await expect(page.getByRole('link', { name: 'Go to allowed route' })).toHaveAttribute('href', '/run-lane')
})
