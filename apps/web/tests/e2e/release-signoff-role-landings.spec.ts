...
function isIgnorableConsoleNoise(line: string) {
  return /404|favicon/i.test(line) || /Failed to load resource: net::ERR_CONNECTION_REFUSED/i.test(line)
}

function isIgnorableRequestFailure(line: string) {
  return /favicon/i.test(line)
}
...
async function assertStableLanding(page: Page, item: LandingCase) {
  ...
  const loginButton = page.getByRole('button', { name: /Đăng nhập|sign in/i })
...
const cases: LandingCase[] = [
  { role: 'SUPER_ADMIN', path: '/overview', screenshot: 'landing-super-admin.png' },
  { role: 'SITE_ADMIN', path: '/overview', screenshot: 'landing-site-admin.png' },
  { role: 'MANAGER', path: '/overview', screenshot: 'landing-manager.png' },
  { role: 'OPERATOR', path: '/run-lane', screenshot: 'landing-operator.png' },
  { role: 'GUARD', path: '/run-lane', screenshot: 'landing-guard.png' },
  { role: 'CASHIER', path: '/reports', screenshot: 'landing-cashier.png' },
  { role: 'VIEWER', path: '/overview', screenshot: 'landing-viewer.png' },
]

function usernameForRole(role: Role) {
  switch (role) {
    case 'SUPER_ADMIN':
      return 'super_admin'
    case 'SITE_ADMIN':
...
for (const item of cases) {
  test(`${item.role} login lands on the correct canonical home`, async ({ page }) => {
    ...
    await page.getByLabel(/Tên đăng nhập|Username/i).fill(usernameForRole(item.role))
    ...
    expect(
      diagnostics.requestFailures.filter((line) => !isIgnorableRequestFailure(line)),
      `Unexpected request failures on ${item.role}: ${diagnostics.requestFailures.join(' | ')}`,
    ).toEqual([])

    expect(
      diagnostics.consoleErrors.filter((line) => !isIgnorableConsoleNoise(line)),
