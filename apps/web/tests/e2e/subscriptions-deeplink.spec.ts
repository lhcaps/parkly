...
async function mockAuthenticatedOps(page: Page) {
  ...
  await page.route('**/api/sites**', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({ rows: [{ siteId: 'site-1', siteCode: 'SITE01', name: 'Site 01', timezone: 'Asia/Ho_Chi_Minh', isActive: true }] }),
    })
  })

  await page.route('**/api/me', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ role: 'OPERATOR' }) })
  })
...
test('subscriptions deep link survives reload and keeps the selected tab', async ({ page }) => {
  ...
  await expect(page.locator('h1').filter({ hasText: /subscriptions/i })).toBeVisible({ timeout: 15000 })
