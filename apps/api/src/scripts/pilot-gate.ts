...
export function buildPilotEvidenceFilePath(kind: string, env: NodeJS.ProcessEnv = process.env) {
  return path.join(resolvePilotEvidenceRoot(env), `${kind}.json`)
}
...
export async function performPilotGate(options?: {
  ...
  if (profile.name !== 'RELEASE_CANDIDATE') {
    ...
    writeJson(buildPilotEvidenceFilePath('pilot-gate-summary', env), summary)
  ...
  const secretsArtifact = buildPilotEvidenceFilePath('security-secrets-check', env)
  ...
  const rotationArtifact = buildPilotEvidenceFilePath('security-rotation-check', env)
  ...
  if (verifyBeforeGate) {
    ...
    const verifyArtifact = buildPilotEvidenceFilePath('verify-deployment-pilot', env)
