...
export function registerSpotOccupancyRoutes(api: Router) {
  api.get('/ops/spot-occupancy', requireAuth(ADMIN_OPS_GUARD_ROLES), async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  api.get('/ops/spot-occupancy/:spotCode', requireAuth(ADMIN_OPS_GUARD_ROLES), async (req: Request, res: Response, next: NextFunction) => {
    try {
