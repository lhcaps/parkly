...
import { processApprovedEntryTx } from '../process-entry'
...
export async function manualApproveGateSession(args: {
  ...
  const result = await withAuditActor(args.actor.actorUserId, async (tx) => {
    ...
    await ensureMutableSessionStatus(session, {
      message: 'Session hiện tại không cho phép manual approve',
    ...
    const site = await tx.parking_sites.findUnique({ where: { site_id: session.site_id } })
    if (!lane || !site) throw new ApiError({ code: 'NOT_FOUND', message: 'Không resolve được lane/site cho session' })
    ...
    const decisionInputSnapshot = {
      ...jsonSafe(decision.inputSnapshot),
      actor: auditActorSnapshot(args.actor),
      manualNote: args.note.trim(),
      beforeSnapshot,
    }
    ...
    await tx.gate_decisions.create({
      data: {
        ...
        input_snapshot_json: jsonSafe(decisionInputSnapshot),
        threshold_snapshot_json: jsonSafe({
          ...decision.thresholdSnapshot,
          source: 'PR10_MANUAL_APPROVE',
        }),
      },
    })

    if (String(session.direction) === 'ENTRY') {
      await processApprovedEntryTx({
        tx,
        siteId: session.site_id,
        siteCode: String(site.site_code),
        laneId: session.lane_id,
        laneCode: String(lane.lane_code),
        primaryDeviceId: lane.primary_device_id,
        sessionId,
        occurredAt,
        requestId: `manual-approve:${sessionId.toString()}`,
        decision: {
          ...decision,
          openTicket: null,
          ...
          deviceHealthSnapshot: { status: null, reportedAt: null, ageSeconds: null, health: 'UNKNOWN' },
      ...
      await processApprovedExitTx({
        ...
        decision: {
          ...decision,
          openTicket: session.ticket_id == null
            ? null
            : {
          ...
          activePresence: null,
          credentialStatus: 'UNKNOWN',
          plateTicketId: session.ticket_id == null ? null : String(session.ticket_id),
          rfidTicketId: session.ticket_id == null ? null : String(session.ticket_id),
        },
        plateCompact: session.plate_compact ?? null,
        rfidUid: session.rfid_uid ?? null,
        readEventId: null,
    ...
    await tx.gate_manual_reviews.update({
      ...
      data: {
        ...
        resolved_at: occurredAt,
        note: args.note.trim(),
        snapshot_json: appendAuditTrail(review.snapshot_json, auditEntry),
      },
    })
    ...
    await tx.gate_incidents.create({
      data: {
        ...
        severity: 'INFO',
        status: 'ACKED',
        incident_type: 'MANUAL_APPROVE',
        title: args.reasonCode.trim(),
        detail: args.note.trim(),
        snapshot_json: jsonSafe(auditEntry),
        resolved_at: occurredAt,
      },
    })
  ...
  await writeReviewAuditLog({
    ...
    action: 'GATE_REVIEW_MANUAL_APPROVE',
    actor: args.actor,
