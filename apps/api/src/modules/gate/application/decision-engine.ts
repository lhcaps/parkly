...
import { findActivePresenceConflicts } from '../../../server/services/presence-service'
import { findCredentialByRfid, resolveOpenTicketForDecision } from '../../../server/services/ticket-service'
import {
  buildManualDecision,
...
export async function evaluateSessionDecision(args: DecisionEngineEvalInput): Promise<DecisionEngineEvalResult> {
  ...
  const output = rawOutput.decisionCode === 'ANTI_PASSBACK_BLOCKED'
    ...
    ? buildManualDecision({
        ...
        reasonDetail: 'ALPR chÆ°a Ä‘á»§ cháº¯c Ä‘á»ƒ hard deny anti-passback. Session Ä‘Æ°á»£c Ä‘áº©y sang review Ä‘á»ƒ operator xÃ¡c nháº­n.',
      })
    : rawOutput
  ...
  return {
    ...
    thresholdSnapshot: jsonSafe({
      ...
      engineVersion: 'PR08_EXIT_FLOW_V1',
    }),
    openTicket: openTicketContext.openTicket,
    paymentStatus,
    paymentResolution,
...
export function buildManualApproveDecision(reasonCode?: string, reasonDetail?: string) {
  return buildManualDecision({
    ...
    reasonDetail: reasonDetail?.trim() || 'Operator Ä‘Ã£ approve thá»§ cÃ´ng tá»« session UI.',
  })
}
...
export function buildManualDenyDecision(reasonCode?: string, reasonDetail?: string) {
  return buildManualDecision({
    ...
    reasonDetail: reasonDetail?.trim() || 'Operator Ä‘Ã£ deny thá»§ cÃ´ng tá»« session UI.',
  })
}
...
export function buildManualPaymentHoldDecision(reasonCode?: string, reasonDetail?: string) {
  return buildManualDecision({
    ...
    reasonDetail: reasonDetail?.trim() || 'Operator Ä‘Ã£ giá»¯ session á»Ÿ WAITING_PAYMENT.',
  })
}
