...
export type DeviceCaptureAuthResult = {
  verified: boolean;
  secretSource: 'DEVICE_CAPTURE_SECRETS_JSON' | 'DEVICE_CAPTURE_SECRET_<DEVICE_CODE>' | 'DEVICE_CAPTURE_DEFAULT_SECRET' | 'DISABLED';
  timestampIso: string;
...
export function resolveDeviceSecret(deviceCode: string): {
  ...
  const defaultSecret = normalizeText(process.env.DEVICE_CAPTURE_DEFAULT_SECRET);
  if (defaultSecret) {
    return { secret: defaultSecret, source: 'DEVICE_CAPTURE_DEFAULT_SECRET' };
  }
...
export function verifyDeviceSignature(args: VerifyDeviceSignatureInput): DeviceCaptureAuthResult {
  ...
  const { secret, source } = resolveDeviceSecret(args.deviceCode);
  const expected = buildDeviceSignature({ ...args, secret }).toLowerCase();

  if (!timingSafeHexCompare(expected, signature)) {
    throw new ApiError({
      code: 'UNAUTHENTICATED',
