...
export async function findReusableSession(args: {
  ...
  // SELECT ... FOR UPDATE locks the selected row within the transaction,
  // serializing concurrent requests that attempt to reuse the same stale session.
  const candidate = await (tx ?? prisma).$queryRaw<any[]>`
    SELECT gps.session_id, gps.status, gps.opened_at, gps.last_read_at, gps.plate_compact, gps.rfid_uid
    FROM gate_passage_sessions gps
    WHERE gps.site_id = ${args.siteId}
      AND gps.lane_id = ${args.laneId}
      AND gps.direction = ${asGateDirection(args.direction)}
      AND gps.status IN (${Prisma.join(ACTIVE_SESSION_STATUSES)})
    ORDER BY gps.last_read_at DESC, gps.opened_at DESC, gps.session_id DESC
    LIMIT 1
    FOR UPDATE
  `;
  ...
  if (!shouldReuseSession({
    ...
    now: args.now,
    windowMs: args.reuseWindowMs,
  })) {
    return null;
  }

  return {
    session_id: BigInt(row.session_id),
    status: String(row.status),
    opened_at: row.opened_at,
    last_read_at: row.last_read_at,
    plate_compact: row.plate_compact,
