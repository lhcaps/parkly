...
export async function listGateSessions(args: {
  ...
  if (args.laneCode?.trim()) {
    where.push('gl.lane_code = ?');
    params.push(args.laneCode.trim());
  }

  if (args.status?.trim()) {
    where.push('gps.status = ?');
    params.push(args.status.trim());
  }

  if (args.direction) {
    where.push('gps.direction = ?');
    params.push(args.direction);
  }

  if (args.from) {
    where.push('gps.opened_at >= ?');
