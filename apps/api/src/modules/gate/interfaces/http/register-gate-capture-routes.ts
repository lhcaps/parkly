...
import { withLaneLock } from '../../../../lib/lane-lock';
...
async function claimOrReplay(args: {
  ...
  throw new ApiError({
    ...
    details: { reason: 'IDEMPOTENCY_BUSY', scope: args.scope, status: claimed.row.status },
  });
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Helper: idempotency-safe ingest for all capture types
// ─────────────────────────────────────────────────────────────────────────────
type CaptureIngestFn<T> = (opts: {
  idempotencyKey: string;
...
async function runWithLaneLockAndIdempotency<T>(opts: {
  req: any;
  rid: string;
  scope: string;
  body: Record<string, unknown>;
  ...
  laneCode: string;
  ingestFn: CaptureIngestFn<T>;
  captureCtx: CaptureLogContext;
  readType: DeviceCaptureReadType;
}) {
  ...
  if (claim.replay) {
    ...
    logCapture(opts.req, 'info', 'capture_replay', opts.captureCtx);
    return { replay: true, responseJson: claim.responseJson };
  }
  claimedByThisRequest = true;

  // ── Distributed Lane Lock ─────────────────────────────────────────────
  // Serialize Camera vs RFID vs Sensor on the same lane.
  // Lock TTL (3s default) covers Capture → Decision Engine cycle.
  const result = await withLaneLock(
    { siteCode: opts.body.siteCode as string, laneCode: opts.laneCode, traceId: opts.rid },
    async () => {
      // Inside lock: idempotency re-check (another thread may have succeeded
      // during our brief wait for the lock)
      const recheck = await claimOrReplay({
        scope: opts.scope,
        idempotencyKey: opts.idempotencyKey,
        requestHash: opts.requestHash,
      });
      ...
      if (recheck.replay) {
        observeSecretReplaySuspicion({ channel: 'DEVICE_CAPTURE', reason: 'IDEMPOTENCY_REPLAY' });
        logCapture(opts.req, 'warn', 'capture_replay_inside_lock', opts.captureCtx);
        return { replay: true, responseJson: recheck.responseJson };
      }

      const data = await opts.ingestFn({
        idempotencyKey: buildPersistedIdempotencyKey(
          opts.readType,
          opts.body.siteCode as string,
          opts.body.deviceCode as string,
          opts.idempotencyKey,
        ),
        body: opts.body,
        auth: opts.auth,
      ...
      pushCaptureFeed({
        ...
        requestId: opts.body.requestId as string,
      ...
      await markIdempotencySucceeded({
        ...
        responseJson: stringifyBigint(data),
      });

      return { replay: false, responseJson: stringifyBigint(data) };
    },
  );
...
export function registerGateCaptureRoutes(api: Router) {
  ...
  api.post('/gate-reads/alpr', async (req, res, next) => {
    ...
    const captureCtx: CaptureLogContext = { rid, scope: '' };
    ...
    try {
      ...
      captureCtx.scope = '';
      captureCtx.siteCode = body.siteCode;
      captureCtx.deviceCode = body.deviceCode;
      captureCtx.deviceRequestId = body.requestId;
      captureCtx.idempotencyKey = body.idempotencyKey;
      ...
      const scope = buildCaptureScope('ALPR', body.siteCode, body.deviceCode);
      captureCtx.scope = scope;
      const laneCode = body.laneCode ?? body.direction;

      const result = await runWithLaneLockAndIdempotency({
        req,
        rid,
        scope,
        ...
        auth: {
          ...
          timestampIso: auth.timestampIso,
          maxSkewSeconds: auth.maxSkewSeconds,
        },
        laneCode,
        captureCtx,
        readType: 'ALPR',
        ingestFn: async ({ idempotencyKey }) => ingestAlprRead({
          requestId: body.requestId,
          idempotencyKey,
          siteCode: body.siteCode,
          laneCode: body.laneCode,
      ...
      res.json(ok(rid, result.responseJson));
    } catch (error) {
  ...
  api.post('/gate-reads/rfid', async (req, res, next) => {
    ...
    try {
      ...
      const occurredAt = body.eventTime ? new Date(body.eventTime) : new Date(auth.timestampIso);
      const requestHash = hashRequest({ ...body, signature: undefined });
      const scope = buildCaptureScope('RFID', body.siteCode, body.deviceCode);
      captureCtx.scope = scope;
      const laneCode = body.laneCode ?? body.direction;
      ...
      const result = await runWithLaneLockAndIdempotency({
        ...
        captureCtx,
        readType: 'RFID',
        ingestFn: async ({ idempotencyKey }) => ingestRfidRead({
          requestId: body.requestId,
          idempotencyKey,
          siteCode: body.siteCode,
          laneCode: body.laneCode,
  ...
  api.post('/gate-reads/sensor', async (req, res, next) => {
    ...
    try {
      ...
      const occurredAt = body.eventTime ? new Date(body.eventTime) : new Date(auth.timestampIso);
      const requestHash = hashRequest({ ...body, signature: undefined });
      const scope = buildCaptureScope('SENSOR', body.siteCode, body.deviceCode);
      captureCtx.scope = scope;
      const laneCode = body.laneCode ?? body.direction;
      ...
      const result = await runWithLaneLockAndIdempotency({
        ...
        captureCtx,
        readType: 'SENSOR',
        ingestFn: async ({ idempotencyKey }) => ingestSensorRead({
          requestId: body.requestId,
          idempotencyKey,
          siteCode: body.siteCode,
          laneCode: body.laneCode,
  ...
  // ── Heartbeat ──────────────────────────────────────────────────────────
  // Heartbeat không cần lane lock (không ảnh hưởng session creation)
