...
export function registerGateOpsQueryRoutes(api: Router) {
  api.get('/ops/lane-status', requireAuth(GATE_MONITOR_READ_ROLES), async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  api.get('/ops/device-health', requireAuth(GATE_MONITOR_READ_ROLES), async (req: Request, res: Response, next: NextFunction) => {
    try {
      const parsed = validateOrThrow(SnapshotQuery, req.query ?? {})
      const scope = await resolveAuthorizedSiteScope({
        principal: req.auth!,
