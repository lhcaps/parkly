...
import { AcceptedAuthRoleSchema } from '@parkly/contracts'
import { z } from 'zod'
...
const LoginBodySchema = z.object({
  ...
  role: AcceptedAuthRoleSchema.optional(),
})
...
export function registerAuthRoutes(api: Router) {
  ...
  api.post('/auth/revoke-all', requireAuth(ALL_CANONICAL_USER_ROLES), async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  api.post('/auth/admin/users/:userId/revoke-all', requireAuth(ADMIN_OPS_ROLES), async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  api.post('/auth/admin/users/:userId/disable', requireAuth(ADMIN_OPS_ROLES), async (req: Request, res: Response, next: NextFunction) => {
    try {
      const params = validateOrThrow(AdminUserParamsSchema, req.params ?? {})
      const parsed = validateOrThrow(AdminUserMutationBodySchema, req.body ?? {})
  ...
  api.post('/auth/admin/users/:userId/enable', requireAuth(ADMIN_OPS_ROLES), async (req: Request, res: Response, next: NextFunction) => {
    try {
      const params = validateOrThrow(AdminUserParamsSchema, req.params ?? {})
      const parsed = validateOrThrow(AdminUserMutationBodySchema, req.body ?? {})
      const user = await authService.setUserStatus({
  ...
  api.get('/auth/me', requireAuth(ALL_CANONICAL_USER_ROLES), async (req: Request, res: Response) => {
