...
const APP_ROLE_SET = new Set<AppRole>(['ADMIN', 'OPS', 'GUARD', 'CASHIER', 'WORKER'])
...
function pickRole(userRoles: AppRole[], requestedRole?: AppRole | null) {
  if (requestedRole) {
...
function serializeActorLabel(role: AppRole, userId: string) {
  return `${role}:${userId}`
}
...
function buildUserPrincipal(user: AuthUserRecord, sessionId: string, role: AppRole): AuthenticatedUserPrincipal {
  return {
    principalType: 'USER',
    role,
...
async function loadRolesForUser(userId: string): Promise<AppRole[]> {
  ...
  return rows
    .map((row) => asAppRole(row.roleCode))
...
function mapSessionRow(row: {
  ...
  const role = asAppRole(row.roleCode)
  if (!role) return null
  return {
  ...
  async function authenticateAccessToken(token: string, meta: { ipAddress?: string | null; userAgent?: string | null } = {}): Promise<AuthenticatedPrincipal> {
    if (config.authMode === 'OFF') {
      ...
      return {
        ...
        actorLabel: `ADMIN:${fallbackActor.toString()}`,
    ...
    if (config.allowLegacyRoleTokens) {
      const legacyRole = (Object.keys(config.tokens) as AppRole[]).find((role) => config.tokens[role] && token === config.tokens[role])
      if (legacyRole) {
        const actorUserId = config.actors[legacyRole] ?? 0n
        return {
          principalType: 'USER',
          role: legacyRole,
          actorUserId,
          actorLabel: `${legacyRole}:${actorUserId.toString()}`,
          userId: actorUserId.toString(),
          username: `legacy-${legacyRole.toLowerCase()}`,
          sessionId: `legacy-${legacyRole.toLowerCase()}`,
          siteScopes: [],
        }
      }
    }
