...
export function registerAuditRoutes(api: Router) {
  api.get('/ops/audit', requireAuth(ADMIN_OPS_ROLES), async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  api.get('/ops/audit/:auditId', requireAuth(ADMIN_OPS_ROLES), async (req: Request, res: Response, next: NextFunction) => {
    try {
