...
export const config = (() => {
  const schema = z.object({
    ...
    API_INTERNAL_SERVICE_ROLE: z.enum(['ADMIN', 'OPS', 'GUARD', 'CASHIER', 'WORKER']).default('WORKER'),
  ...
  const internalServiceToken = getPrimaryRotationSecretValue(internalServiceRotation) ?? parsed.API_INTERNAL_SERVICE_TOKEN ?? 'internal_service_dev_token_change_me'
  ...
  return {
    ...
    allowLegacyRoleTokens: parsed.API_LEGACY_ROLE_TOKENS === 'ON',
    auth: {
