...
import { DependencyUnavailableError } from '../http'
...
function storageErrorMessage(error: unknown) {
  return String((error as { message?: unknown } | null | undefined)?.message ?? error ?? 'Unknown object storage error').trim()
}
    ...
    try {
      ...
      throw new DependencyUnavailableError('Object storage', {
        ...
        details: {
          ...
          objectKey,
          driver: 'MINIO',
          reason: storageErrorMessage(error),
        },
      })
    }
  }
