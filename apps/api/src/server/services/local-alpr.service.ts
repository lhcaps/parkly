...
export function resolveLocalUploadFromUrl(imageUrl?: string | null): { absolutePath: string; relativePath: string; filename: string } | null {
...
export async function recognizeLocalPlate(args: { imageUrl?: string | null; plateHint?: string | null }): Promise<LocalAlprRecognition> {
  ...
  const local = resolveLocalUploadFromUrl(args.imageUrl)
  if (!local) {
    throw new ApiError({
      code: 'BAD_REQUEST',
      message: 'imageUrl phải trỏ tới ảnh upload local hợp lệ hoặc truyền plateHint.',
      details: {
  ...
  const meta = await readImageMeta(local.absolutePath)
  ...
  if (config.alpr.mode === 'MOCK') {
    ...
    const shouldRunDeep = !ranked.candidates[0] || ranked.candidates[0].previewStatus !== 'STRICT_VALID' || ranked.candidates[0].score < config.alpr.externalEscalationThreshold
    if (shouldRunDeep) {
  ...
  const source = winner?.provider && winner.provider !== 'LOCAL' ? 'HTTP_PROVIDER' : 'LOCAL_OCR'
