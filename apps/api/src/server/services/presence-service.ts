...
/**
 * Computes presence_hash per V26 schema: SHA2-256 of the composite key.
 * This enables a unique constraint even when ticket_id/plate_compact/rfid_uid
 * contain NULL values — a known MySQL limitation with multi-column UNIQUE.
 *
 * Pattern: site_id | ticketId | plateCompact | rfidUid
 * Null components are represented as empty string in the hash input.
 */
export function computePresenceHash(args: {
  siteId: bigint
  ...
  const components = [
    String(args.siteId),
    args.ticketId != null ? String(args.ticketId) : '',
    args.plateCompact ?? '',
    args.rfidUid ?? '',
  ]
...
export async function upsertActivePresenceTx(tx: Tx, args: {
  ...
  const presenceHash = computePresenceHash({
    siteId: args.siteId,
