...
export async function buildApp() {
  ...
  api.get('/me', requireAuth(['ADMIN', 'OPS', 'GUARD', 'WORKER'] as AppRole[]), (req, res) => {
    ...
    res.json(ok(rid, { role: (req as any).auth?.role ?? 'ADMIN' }));
  });
  ...
  api.get('/sites', requireAuth(['ADMIN', 'OPS', 'GUARD', 'WORKER']), async (req, res, next) => {
    try {
      ...
      if (!parsed.data.includeInactive) rows = rows.filter((row) => row.isActive);
  ...
  api.get('/gates', requireAuth(['ADMIN', 'OPS', 'GUARD', 'WORKER']), async (req, res, next) => {
    try {
      const parsed = MasterDataQuery.safeParse(req.query ?? {});
      if (!parsed.success) throw new ApiError({ code: 'BAD_REQUEST', details: parsed.error.flatten() });

      const siteCode = parsed.data.siteCode;
      const master = await getMasterDataFromLanes(siteCode);
  ...
  api.get('/devices', requireAuth(['ADMIN', 'OPS', 'GUARD', 'WORKER']), async (req, res, next) => {
    try {
      const parsed = DeviceListQuery.safeParse(req.query ?? {});
      if (!parsed.success) throw new ApiError({ code: 'BAD_REQUEST', details: parsed.error.flatten() });

      let rows = await getLatestDeviceRows(parsed.data.siteCode);
      if (parsed.data.heartbeatStatus) rows = rows.filter((row) => row.heartbeatStatus === parsed.data.heartbeatStatus);
      if (parsed.data.unassignedOnly) rows = rows.filter((row) => !row.laneCode);

      const rid = (req as any).id;
      const data = DevicesListResponseSchema.parse({ siteCode: parsed.data.siteCode ?? null, rows });
      res.json(ok(rid, data));
    } catch (e) {
      next(e);
    }
  ...
  api.get('/topology', requireAuth(['ADMIN', 'OPS', 'GUARD', 'WORKER']), async (req, res, next) => {
    try {
      ...
      const data = await buildTopology(parsed.data.siteCode);
      const rid = (req as any).id;
      res.json(ok(rid, { siteCode: parsed.data.siteCode, ...data }));
    } catch (e) {
      next(e);
    }
  });
  ...
  api.get('/outbox', requireAuth(['ADMIN', 'OPS', 'WORKER']), async (req, res, next) => {
    try {
      const parsed = OutboxListQuery.safeParse(req.query ?? {});
      if (!parsed.success) throw new ApiError({ code: 'BAD_REQUEST', details: parsed.error.flatten() });

      const siteId = parsed.data.siteCode ? await resolveSiteIdByCode(parsed.data.siteCode) : undefined;
      const cursor = parsed.data.cursor ? BigInt(parsed.data.cursor) : undefined;
      ...
      await refreshBusinessMetrics(parsed.data.siteCode);
      ...
      res.json(ok((req as any).id, stringifyBigint({
        rows: data.items.map(sanitizeOutboxItem),
  ...
  api.post('/outbox/drain', requireAuth(['ADMIN', 'WORKER']), async (req, res, next) => {
    try {
      const parsed = OutboxDrainBody.safeParse(req.body ?? {});
      if (!parsed.success) throw new ApiError({ code: 'BAD_REQUEST', details: parsed.error.flatten() });

      const data = await drainOutboxOnce({ limit: parsed.data.limit, dryRun: parsed.data.dryRun });
      await refreshBusinessMetrics(undefined);
      (req as any).log?.info?.({
        requestId: (req as any).id,
        correlationId: (req as any).correlationId,
  ...
  api.post('/outbox/requeue', requireAuth(['ADMIN', 'OPS']), async (req, res, next) => {
    try {
      const parsed = OutboxRequeueBody.safeParse(req.body ?? {});
      if (!parsed.success) throw new ApiError({ code: 'BAD_REQUEST', details: parsed.error.flatten() });
      const ids = parsed.data.outboxIds?.map((value) => BigInt(value));
      const changed = await requeueOutbox({ outboxIds: ids, limit: parsed.data.limit });
      await refreshBusinessMetrics(undefined);
      (req as any).log?.warn?.({
        requestId: (req as any).id,
        correlationId: (req as any).correlationId,
  ...
  api.post('/gate-events', requireAuth(['ADMIN', 'OPS', 'GUARD']), async (req, res, next) => {
    try {
      const parsed = GateEventWriteBodySchema.safeParse(req.body);
      if (!parsed.success) {
  ...
  api.get('/gate-events', requireAuth(['ADMIN', 'OPS', 'GUARD', 'WORKER']), async (req, res, next) => {
    try {
      const parsed = GateEventListQuery.safeParse(req.query ?? {});
      if (!parsed.success) throw new ApiError({ code: 'BAD_REQUEST', details: parsed.error.flatten() });

      const siteCode = parsed.data.siteCode ?? (await resolveDefaultSiteCode());
      const siteId = await resolveSiteIdByCode(siteCode);
      const deviceId = parsed.data.deviceCode
        ? await resolveDeviceIdByCode({ siteId, deviceCode: parsed.data.deviceCode })
        : undefined;
      ...
      const where: any = {
        ...
        ...(parsed.data.sinceTime ? { event_time: { gte: new Date(parsed.data.sinceTime) } } : {}),
        ...(parsed.data.sinceEventId ? { event_id: { gt: BigInt(parsed.data.sinceEventId) } } : {}),
      };

      const orderBy = parsed.data.sinceEventId ? { event_id: 'asc' as const } : { event_id: 'desc' as const };
      ...
      const rid = (req as any).id;
      res.json(ok(rid, { siteCode, rows: items }));
    } catch (e) {
      next(e);
    }
  });
...
<body>
  ...
  api.post('/lane-flow/submit', requireAuth(['ADMIN', 'OPS', 'GUARD']), async (req, res, next) => {
    const rid = (req as any).id;
    try {
      const parsed = LaneFlowSubmitBodySchema.safeParse(req.body);
      if (!parsed.success) throw new ApiError({ code: 'BAD_REQUEST', details: parsed.error.flatten() });
