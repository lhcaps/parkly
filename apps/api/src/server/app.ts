...
import {
  ...
  ADMIN_OPS_GUARD_CASHIER_WORKER_ROLES,
  ADMIN_OPS_GUARD_ROLES,
  ADMIN_OPS_GUARD_WORKER_ROLES,
  ADMIN_OPS_ROLES,
  ADMIN_OPS_WORKER_ROLES,
} from './auth-policies';
...
import { ApiError, buildCursorPageInfo, defaultMessageForCode, DependencyUnavailableError, fail, ok, statusToCode, withCursorPage } from './http';
import { getRequestActor, requireAuth } from './auth';
import { filterRowsBySiteCodes, resolveAuthorizedSiteScope } from './services/read-models/site-scope';
...
import { serializePrincipal } from '../modules/auth/application/auth-service';
...
import { registerEdgeRoutes } from '../modules/edge/interfaces/http/register-edge-routes';
...
import { createMobilePairing, getMobilePairing, invalidateMobilePairing, logMobilePairingInfo } from './services/mobile-pairing.service';
...
import { registerMediaRoutes } from '../modules/media/interfaces/http/register-media-routes';
import { registerBulkImportRoutes } from '../modules/bulk-import/interfaces/http/register-bulk-import-routes';
import { registerWebhookRoutes } from '../modules/webhooks/interfaces/http/register-webhook-routes';
...
function deduplicateBy<T extends Record<string, unknown>>(arr: T[], key: keyof T): T[] {
...
function buildMasterDataFromDevices(siteCode: string, devices: Array<any>): { gates: GateRow[]; lanes: LaneRow[] } {
  // Group devices by (site, gateCode, direction) to create unique lanes
  const laneMap = new Map<string, { siteCode: string; gateCode: string; laneCode: string; label: string; direction: string; devices: any[] }>();
  ...
  for (const d of devices) {
    ...
    const key = `${siteCode}|${gateCode}|${direction}`;

    if (!laneMap.has(key)) {
      laneMap.set(key, {
        siteCode,
        gateCode,
        laneCode: `${gateCode}_${direction}`,
        label: `${gateCode} - ${direction}`,
        direction,
        devices: [],
      });
    }
    laneMap.get(key)!.devices.push(d);
  }

  const laneRows: LaneRow[] = Array.from(laneMap.values()).map((lane) => ({
    siteCode: lane.siteCode,
    gateCode: lane.gateCode,
    laneCode: lane.laneCode,
    label: lane.label,
    direction: lane.direction as 'ENTRY' | 'EXIT',
    deviceCode: lane.devices[0]?.device_code ? String(lane.devices[0].device_code) : '',
    deviceType: lane.devices[0]?.device_type ? String(lane.devices[0].device_type) : '',
    locationHint: lane.devices[0]?.location_hint ? String(lane.devices[0].location_hint) : null,
    primaryDeviceCode: lane.devices[0]?.device_code ? String(lane.devices[0].device_code) : '',
  }));
...
async function getMasterDataFromLanes(siteCode: string): Promise<{ gates: GateRow[]; lanes: LaneRow[] } | null> {
  ...
  const rows = await prisma.$queryRawUnsafe<Array<Record<string, unknown>>>(
    `
      SELECT
        ...
        z.name AS zoneName,
        NULL AS floorKey,
        lane_spots.spot_count AS spotCount
      FROM gate_lanes gl
      JOIN parking_sites ps
        ON ps.site_id = gl.site_id
      LEFT JOIN zones z
      ...
      LEFT JOIN (
        SELECT sp.site_id, z2.code, COUNT(sp.spot_id) AS spot_count
        FROM spots sp
        JOIN zones z2 ON z2.zone_id = sp.zone_id
        GROUP BY sp.site_id, z2.code
      ) lane_spots ON lane_spots.site_id = ps.site_id AND lane_spots.code = z.code
      LEFT JOIN gate_lane_devices gld_primary
        ON gld_primary.lane_id = gl.lane_id
       AND gld_primary.is_primary = 1
      LEFT JOIN gate_devices gd_primary
  ...
  const lanes = LanesListResponseSchema.shape.rows.parse(rows.map((row: Record<string, unknown>) => ({
    ...
    sortOrder: Number(row.sortOrder ?? 0),
    primaryDeviceCode: row.primaryDeviceCode == null ? null : String(row.primaryDeviceCode),
    zoneCode: row.zoneCode == null ? undefined : String(row.zoneCode),
    zoneName: row.zoneName == null ? undefined : String(row.zoneName),
    floorKey: row.floorKey == null ? undefined : String(row.floorKey),
    spotCount: row.spotCount == null ? undefined : Number(row.spotCount),
  })).filter(row => row.deviceCode.length > 0));

  return {
    gates: GatesListResponseSchema.shape.rows.parse(deduplicateBy(buildGateRowsFromLanes(lanes), 'gateCode')),
    lanes: LanesListResponseSchema.shape.rows.parse(deduplicateBy(lanes, 'laneCode')),
  };
}
...
export async function buildApp() {
  ...
  (app as any).locals.secretRedactionPaths = [
  ...
  app.get('/openapi.json', (_req, res) => {
    if (process.env.NODE_ENV === 'production') {
      res.status(403).json({ code: 'FORBIDDEN', message: 'OpenAPI spec is not available in production' })
      return
    }
    res.json(openapi)
  })
  app.use('/docs', (_req, _res, next) => {
    if (process.env.NODE_ENV === 'production') {
      _res.status(403).json({ code: 'FORBIDDEN', message: 'Swagger docs are not available in production' })
      return
    }
    next()
  ...
  // ─── Role definitions (production DB roles + legacy code aliases) ────────
  // SUPER_ADMIN bypasses ALL role checks (see requireAuth in server/auth.ts).
  const ADMIN_OPS = ADMIN_OPS_ROLES;

  // ── Rate limiter for public mobile capture endpoints ──────────────────────
  // Limits pair-token operations per IP (defense-in-depth against token enumeration).
  const mobileCaptureRateLimiter = rateLimit({
    windowMs: 60_000,
    max: 20,
    standardHeaders: true,
    legacyHeaders: false,
    keyGenerator: (req: any) => String(req.ip ?? req.header('x-forwarded-for') ?? 'unknown'),
    handler: (_req: any, res: any) => {
      res.status(429).json({
        requestId: _req.id ?? 'unknown',
        code: 'RATE_LIMITED',
        message: 'Too many mobile capture requests from this IP. Retry after 60 seconds.',
        details: { retryAfterSeconds: 60 },
      })
    },
  })

  const ADMIN_OPS_GUARD = ADMIN_OPS_GUARD_ROLES;
  const ADMIN_OPS_WORKER = ADMIN_OPS_WORKER_ROLES;
  const ADMIN_OPS_GUARD_WORKER = ADMIN_OPS_GUARD_WORKER_ROLES;
  const ADMIN_OPS_GUARD_CASHIER_WORKER = ADMIN_OPS_GUARD_CASHIER_WORKER_ROLES;
  const ADMIN_OPS_CASHIER = ADMIN_OPS_CASHIER_ROLES;

  api.get('/ops/metrics/summary', requireAuth(ADMIN_OPS), async (req, res, next) => {
    try {
      await refreshBusinessMetrics(undefined);
  ...
  api.get('/me', requireAuth([...ADMIN_OPS_GUARD_CASHIER_WORKER, 'VIEWER']), (req, res) => {
    const rid = (req as any).id;
    res.setHeader('Deprecation', 'true');
    res.json(ok(rid, serializePrincipal(req.auth!)));
  });
  ...
  api.get('/gates', requireAuth([...ADMIN_OPS_GUARD_WORKER, 'VIEWER']), async (req, res, next) => {
    try {
      const parsed = validateOrThrow(MasterDataQuery, req.query ?? {});
      await resolveAuthorizedSiteScope({
        principal: req.auth!,
        requestedSiteCode: parsed.siteCode,
        resourceLabel: 'gates',
      });
  ...
  api.get('/lanes', requireAuth([...ADMIN_OPS_GUARD_WORKER, 'VIEWER']), async (req, res, next) => {
    try {
      const parsed = validateOrThrow(MasterDataQuery, req.query ?? {});
      await resolveAuthorizedSiteScope({
        principal: req.auth!,
  ...
  api.get('/devices', requireAuth([...ADMIN_OPS_GUARD_WORKER, 'VIEWER']), async (req, res, next) => {
    try {
      ...
      let rows = scope.requestedSiteCode
        ? await getLatestDeviceRows(scope.requestedSiteCode)
        : filterRowsBySiteCodes(await getLatestDeviceRows(), scope.siteCodes);
      ...
      const rid = (req as any).id;
      const data = DevicesListResponseSchema.parse({ siteCode: scope.requestedSiteCode ?? null, rows });
      res.json(ok(rid, data));
    } catch (e) {
      next(e);
    }
  ...
  api.get('/topology', requireAuth([...ADMIN_OPS_GUARD_WORKER, 'VIEWER']), async (req, res, next) => {
    try {
      const parsed = validateOrThrow(MasterDataQuery, req.query ?? {});
      await resolveAuthorizedSiteScope({
        principal: req.auth!,
  ...
  api.get('/outbox', requireAuth(ADMIN_OPS_WORKER), async (req, res, next) => {
    try {
      ...
      const siteId = scope.requestedSiteCode ? await resolveSiteIdByCode(scope.requestedSiteCode) : undefined;
      const cursor = parseBigIntCursor(parsed.cursor);
      const sourceData = await listOutbox({
        siteId,
      ...
      const allowedSiteIds = siteId == null
        ? new Set((await Promise.all(scope.siteCodes.map((siteCode) => resolveSiteIdByCode(siteCode)))).map((value) => value.toString()))
        : null;
      const data = {
        ...sourceData,
        items: allowedSiteIds
          ? sourceData.items.filter((item) => item.site_id != null && allowedSiteIds.has(item.site_id.toString()))
          : sourceData.items,
      };
      const items = data.items.map((item) => ({ eventId: item.event_id }));

      await refreshBusinessMetrics(scope.requestedSiteCode ?? undefined);
      ...
      const payload = stringifyBigint(withCursorPage(data.items.map(sanitizeOutboxItem), {
        limit,
        nextCursor: data.nextCursor ?? pageInfo.nextCursor,
        hasMore: data.nextCursor != null,
        sort: pageInfo.sort,
      }));
  ...
  api.post('/outbox/drain', requireAuth(ADMIN_OPS_WORKER), async (req, res, next) => {
    try {
  ...
  api.post('/outbox/requeue', requireAuth(ADMIN_OPS), async (req, res, next) => {
    try {
  ...
  api.post('/gate-events', requireAuth(ADMIN_OPS_GUARD), async (req, res, next) => {
    try {
  ...
  api.get('/gate-events', requireAuth(ADMIN_OPS_GUARD_WORKER), async (req, res, next) => {
    try {
...
<body>
  ...
  <script>
    ...
    // Start periodic session refresh (every 60 seconds)
    setInterval(async () => {
      try {
        await api('/api/mobile-capture/session?pairToken=' + encodeURIComponent(pairToken));
      } catch (error) {
        console.warn('[MobileCapture] Session refresh failed:', error.message);
      }
    }, 60000);
  </script>
</body>
  ...
  api.post('/media/upload', requireAuth(ADMIN_OPS_GUARD), upload.single('file'), async (req, res, next) => {
    try {
  ...
  app.get('/mobile-capture', mobileCaptureRateLimiter, async (req, res, next) => {
    try {
      ...
      if (!pairing) {
        throw new ApiError({ code: 'NOT_FOUND', message: 'Pair token mobile camera không còn hiệu lực hoặc đã hết hạn. Vui lòng tạo pairing mới từ trang quản lý.' });
      }
  ...
  // Mobile capture endpoints - public, uses pairToken for authentication
  // Registered on `api` router (under /api prefix) so they pass through standard middleware
  // and return structured JSON errors instead of 404 from the catch-all.
  api.get('/mobile-capture/session', mobileCaptureRateLimiter, async (req, res, next) => {
    try {
      const pairToken = String((req.query as any)?.pairToken ?? '').trim();
      
      if (!pairToken) {
        throw new ApiError({ code: 'BAD_REQUEST', message: 'Thiếu pairToken để lấy thông tin session' });
      }
      ...
      const rid = (req as any).id;
      res.json(ok(rid, { ...pairing, refreshed: true }));
    } catch (e) {
      next(e);
    }
  });

  api.post('/mobile-capture/pair', requireAuth(ADMIN_OPS_GUARD), async (req, res, next) => {
    try {
  ...
  api.post('/mobile-capture/revoke', requireAuth(ADMIN_OPS_GUARD), async (req, res, next) => {
    try {
      const body = (req.body && typeof req.body === 'object' && !Array.isArray(req.body)) ? req.body as Record<string, unknown> : {};
  ...
  // Mobile capture upload - public, uses pairToken
  api.post('/mobile-capture/upload', mobileCaptureRateLimiter, upload.single('file'), async (req, res, next) => {
    try {
      const pairToken = String((req.query as any)?.pairToken ?? '').trim();
      ...
      logMobilePairingInfo('upload', 'Uploading media from mobile device', {
  ...
  // Mobile capture heartbeat - public, uses pairToken
  api.post('/mobile-capture/heartbeat', mobileCaptureRateLimiter, async (req, res, next) => {
    try {
      const pairToken = String((req.query as any)?.pairToken ?? '').trim();
      ...
      const latencyMs = Number(body.latencyMs ?? 25) || 25;
      
      const heartbeatRequestId = `mobile-hb:${randomUUID()}`;
      
      logMobilePairingInfo('heartbeat', 'Received heartbeat from mobile device', {
      ...
      let result: Awaited<ReturnType<typeof recordDeviceHeartbeat>> | null = null;
      let heartbeatError: { code: string; message: string } | null = null;
      try {
        result = await recordDeviceHeartbeat({
          requestId: heartbeatRequestId,
        ...
        // If device not in DB, return a soft success so mobile camera doesn't keep failing.
        // Log the error but don't block the heartbeat.
        const err = rawErr as any;
        heartbeatError = {
          code: err.code ?? 'DEVICE_CONTEXT_ERROR',
          message: err.message ?? String(rawErr),
        };
        logMobilePairingInfo('heartbeat-error', 'Heartbeat DB write failed (device may not exist in DB)', {
          requestId: heartbeatRequestId,
          pairToken: pairing.pairToken.slice(0, 8) + '...',
          siteCode: pairing.siteCode,
          deviceCode: pairing.deviceCode,
          errorCode: heartbeatError.code,
          errorMessage: heartbeatError.message,
        });
      }

      const rid = (req as any).id;
      res.json(ok(rid, stringifyBigint({
        pairing,
        heartbeat: result,
        refreshed: true,
        heartbeatError: heartbeatError ?? undefined,
      })));
    } catch (e) {
      next(e);
    }
  ...
  // Mobile capture ALPR - public, uses pairToken
  api.post('/mobile-capture/alpr', mobileCaptureRateLimiter, async (req, res, next) => {
    try {
      const pairToken = String((req.query as any)?.pairToken ?? '').trim();
      ...
      const captureRequestId = `mobile-alpr:${randomUUID()}`;
      
      logMobilePairingInfo('alpr-capture', 'Processing ALPR capture from mobile device', {
      ...
      const capture = await ingestAlprRead({
        ...
        rawPayload: {
          ...
          plateHintUsed: usePlateHint,
          plateHintSource: usePlateHint ? plateHintRaw : null,
          tesseractPlate: recognition.recognizedPlate,
          tesseractIsValid,
          tesseractConfidence: recognition.confidence,
        },
      });
      
      logMobilePairingInfo('alpr-capture-complete', 'ALPR capture processed successfully', {
        requestId: captureRequestId,
        pairToken: pairing.pairToken.slice(0, 8) + '...',
        plateRaw: effectivePlateRaw,
        plateHintUsed: usePlateHint,
        confidence: usePlateHint ? 0.99 : recognition.confidence,
      ...
      res.json(ok(rid, stringifyBigint({
        pairing,
        mediaId,
        viewUrl: mediaView?.viewUrl ?? imageUrl,
        recognition,
        capture,
        refreshed: true,
      })));
  ...
  api.post('/device-control/heartbeat-pulse', requireAuth(ADMIN_OPS_GUARD), async (req, res, next) => {
    try {
      const body = (req.body && typeof req.body === 'object' && !Array.isArray(req.body)) ? req.body as Record<string, unknown> : {};
      const siteCode = String(body.siteCode ?? '').trim();
      const deviceCode = String(body.deviceCode ?? '').trim();
    ...
    try {
      ...
      const cached = await resolveAlprPreviewCached(
        ...
        async () => {
          ...
          // Fallback "dữ liệu chạy": khi preview không đọc được biển số nhưng có plateHint thì dùng hint
          const plateHintTrimmed = parsed.plateHint?.trim() ?? '';
          const useHintAsFallback = !hasUsableRecognition && plateHintTrimmed.length > 0;
          const effectivePlate = hasUsableRecognition
            ? recognition.recognizedPlate
            : useHintAsFallback
              ? plateHintTrimmed
              : '';
          const effectiveConfidence = hasUsableRecognition ? recognition.confidence : (useHintAsFallback ? 0.99 : 0);
          const effectiveCandidates = hasUsableRecognition
            ? recognition.candidates
            : useHintAsFallback
              ? [{ plate: plateHintTrimmed.replace(/\s+/g, '').toUpperCase(), score: 99, votes: 0, cropVariants: [], psmModes: [], suspiciousFlags: [] as string[] }]
              : recognition.candidates;

          if (!hasUsableRecognition && !useHintAsFallback && !isPreviewSurface) {
            throw new ApiError({
              code: 'BAD_REQUEST',
              statusCode: 422,
          ...
          const recognizedPlate = effectivePlate;
          ...
          return responseSchema.parse({
            recognizedPlate,
            confidence,
            previewStatus: hasUsableRecognition ? recognition.previewStatus : (useHintAsFallback ? 'NEEDS_REVIEW' : 'INVALID'),
            ...
            winner: recognition.winner
              ...
              : useHintAsFallback
                ? { cropVariant: 'hint', psm: 0, rawText: plateHintTrimmed, score: 99 }
                : null,
            raw: diagnostics,
            ...plateCanonical,
            plate: plateCanonical,
  ...
  api.post('/lane-flow/submit', requireAuth(ADMIN_OPS_GUARD), async (req, res, next) => {
    const rid = (req as any).id;
    try {
  ...
  api.post('/alpr/preview', requireAuth(ADMIN_OPS_GUARD), async (req, res, next) => {
    await handleAlprPreview('POST /api/alpr/preview', req, res, next);
  });

  api.post('/alpr/recognize', requireAuth(ADMIN_OPS_GUARD), async (req, res, next) => {
    await handleAlprPreview('POST /api/alpr/recognize', req, res, next);
  });

  api.get('/reports/summary', requireAuth(ADMIN_OPS_WORKER), async (req, res, next) => {
    try {
  ...
  api.get('/stream/gate-events', requireAuth(ADMIN_OPS_GUARD_WORKER), async (req, res, next) => {
    try {
  ...
  app.use((err: any, req: Request, res: Response, _next: NextFunction) => {
    ...
    if (err instanceof ApiError) {
      ...
      const level = err.statusCode >= 500 ? 'error' : err.statusCode === 409 || err.statusCode === 412 || err.statusCode === 422 || err.statusCode === 401 || err.statusCode === 403 ? 'warn' : 'info';
    ...
    if (err instanceof DependencyUnavailableError) {
      const payload = buildErrorLogPayload(err, req);
      (req as any).log?.error?.({ ...payload, code: 'DEP_UNAVAILABLE', dependency: err.dependency }, 'api_error');
      res.status(503).json(fail(rid, {
        code: 'DEP_UNAVAILABLE',
        message: err.message,
        details: {
          dependency: err.dependency,
          hint: err.dependency === 'Redis'
            ? 'Start Redis or set REDIS_REQUIRED=OFF if Redis is optional.'
            : `Ensure ${err.dependency} is running and reachable.`,
          ...(err.details != null ? { extra: err.details } : {}),
        },
      }));
      return;
    }
    ...
    if (isDbPermissionError(err)) {
      (req as any).log?.error?.({ ...buildErrorLogPayload(err, req), errorCode: 'INTERNAL_ERROR' }, 'db_permission_error');
    ...
    const payload = buildErrorLogPayload(err, req);
    const level = status >= 500 ? 'error' : status === 409 || status === 412 || status === 422 || status === 401 || status === 403 ? 'warn' : 'info';
    (req as any).log?.[level]?.({ ...payload, errorCode: code, status }, 'api_error');
