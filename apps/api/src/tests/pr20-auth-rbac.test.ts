...
class MemoryAuthStore implements AuthStore {
  ...
  seedUser(args: { userId: string; username: string; role: AuthRole; password: string }) {
...
test('login -> me -> refresh -> logout hoạt động và access token cũ bị xoá hiệu lực', async () => {
  await withStableConfig(async () => {
    ...
    store.seedUser({ userId: '101', username: 'admin', role: 'SUPER_ADMIN', password: 'Parkly@123' })
...
test('role matrix và internal service principal được tách riêng', async () => {
  await withStableConfig(async () => {
    const store = new MemoryAuthStore()
    const roles: AuthRole[] = ['SUPER_ADMIN', 'SITE_ADMIN', 'MANAGER', 'CASHIER', 'GUARD', 'OPERATOR', 'VIEWER']
...
test('actor identity được resolve từ principal thay vì env shortcut', async () => {
  ...
  assert.equal(actor.principalType, 'USER')
  assert.equal(actor.role, 'OPERATOR')
  assert.equal(actor.actorUserId, 77n)
  assert.equal(actor.actorLabel, 'OPERATOR:77')
  assert.equal(actor.userId, '77')
})
