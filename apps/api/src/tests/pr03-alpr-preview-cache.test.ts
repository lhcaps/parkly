...
test('preview cache expires by ttl', async () => {
  ...
  writeAlprPreviewCache('preview-key', { ok: true }, 15)
  assert.deepEqual(readAlprPreviewCache('preview-key'), { ok: true })

  await new Promise((resolve) => setTimeout(resolve, 30))
  assert.equal(readAlprPreviewCache('preview-key'), null)
})
