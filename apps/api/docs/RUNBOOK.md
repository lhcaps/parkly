...
Flow smoke canonical cho backend-rc1:

- login -> me -> dashboard -> media -> presence -> reconciliation -> incident -> audit
- auth -> dashboard -> media(local) -> intake -> reconcile -> incident -> audit
- Demo smoke fixture mặc định: SITE_HCM_01 / HCM-VIP-01
- `rc:gate` là shortcut để chạy gate release trước handoff.
...
Label baseline hiện hành là `backend-rc1` và repeat smoke 3 vòng liên tục là điều kiện bắt buộc trước khi chốt RC1.
